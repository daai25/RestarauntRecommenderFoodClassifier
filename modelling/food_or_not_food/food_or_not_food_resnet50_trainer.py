...
from food_or_not_food_custom_model import SimpleCNN
...
def main():
    ...
    if use_resnet:
        ...
        # replace final layer for 2 classes (food and not food)
        in_features = int(model.fc.in_features)

        model.fc = nn.Sequential(  # type: ignore
            nn.Dropout(0.5), nn.Linear(in_features, 2)  # 2 classes: food and not food
        )

        # freeze early layers
        for name, param in model.named_parameters():
            # unfreeze layer4, layer3, layer2, and fc layers
            if (
        ...
        model_path = "custom_best_model.pth"
    ...
    optimizer = optim.Adam(
        [
            ...
            {"params": head_param, "lr": 1e-4, "weight_decay": 1e-5},
        ],
        lr=1e-4,
        weight_decay=1e-4,
    )
...
train_transform = transforms.Compose(
    [
        ...
        transforms.ColorJitter(
            0.2, 0.2, 0.2, 0.1
        ),  # varying brightness/contrast/sat/hue
...
if __name__ == "__main__":
    ...
    if only_testing:
        ...
        model = models.resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)
        in_features = int(model.fc.in_features)
        model.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(in_features, 2))  # type: ignore
        model = model.to(device, memory_format=torch.channels_last)  # type: ignore
